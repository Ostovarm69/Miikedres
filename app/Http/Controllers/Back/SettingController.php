...
class SettingController extends Controller
{
    ...
    public function updateInformation(Request $request)
    {
        ...
        $informations = $request->except(['info_icon', 'info_logo','login_logo']);

        $this->validate($request, [
            'info_site_title'  => 'required',
            'info_icon'        => 'image|max:2048',
            'info_logo'        => 'image|max:2048',
            'login_logo'        => 'image|max:2048',
        ...
        if ($request->hasFile('login_logo')) {
            ...
            $imageName = time() . '_logo.' . $request->login_logo->getClientOriginalExtension();
            $request->login_logo->move(public_path('uploads/'), $imageName);

            $old_logo = option('login_logo');

            if ($old_logo) {
                Storage::disk('public')->delete($old_logo);
            }

            $informations['login_logo'] = '/uploads/' . $imageName;
        }
