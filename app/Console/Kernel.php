...
use App\Jobs\BirthdaySmsToUsers;
...
class Kernel extends ConsoleKernel
{
    ...
    {
            $schedule->command('products:list')
                ->everyMinute()
                ->withoutOverlapping(2)
                ->runInBackground();
            $schedule->command('products:content')
                ->everyFiveMinutes()
                ->withoutOverlapping(6)
                ->runInBackground();
        $schedule->command('products:remove-old-contents')
            ->dailyAt('09:00')
            ->runInBackground();
        ...
        $schedule->job(new BirthdaySmsToUsers)->dailyAt('12:00');
    }
