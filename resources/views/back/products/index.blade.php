...
@section('content')
    ...
    <div class="modal fade" id="products-content-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">محتوای محصول</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
@push('scripts')
    ...
    <script src="{{ asset('back/assets/js/pages/products/index.js') }}?v=5"></script>
    <script src="{{ asset('back/assets/js/pages/products/filters.js') }}?v=2"></script>
    <script>
        $('body').on('click', '.content-data', function (){
            let id = $(this).data('id');
            $('#products-content-modal .modal-body').load('{{ route('admin.product.content.index') }}?id=' + id);
            $('#products-content-modal').modal('show')
        })
    </script>
@endpush
